...
function status_badge(string $status): string
{
    [$class, $label] = match (strtolower($status)) {
        ...
        'pending_user_action'  => ['badge-pending',  'ACTION NEEDED'],
        'pending_verification' => ['badge-pending',  'AWAITING PICKUP'],
...
function sort_link(string $column, string $label, array $state, string $prefix = '', array $base = []): string
{
    ...
    // Inline SVG triangles — immune to emoji-font substitution on Windows/Edge.
    // The parent <span> carries aria-hidden="true" so no role needed on the SVG.
    $svg_up   = '<svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor" focusable="false"><path d="M4 1 7 7H1z"/></svg>';
    $svg_down = '<svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor" focusable="false"><path d="M4 7 1 1h6z"/></svg>';
    $svg_both = '<svg width="8" height="10" viewBox="0 0 8 10" fill="currentColor" focusable="false"><path d="M4 1 7 5H1zM4 9 1 5h6z"/></svg>';
    $indicator = $is_active
        ? ($state['dir'] === 'asc' ? $svg_up : $svg_down)
        : $svg_both;
...
function field_error_html(string $name, array $errors, string $class = 'field-error-text'): string
{
    if (empty($errors[$name])) {
        return '';
    }
