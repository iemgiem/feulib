...
### 6. The app-shell is desktop-only _(partially resolved — 2026-05-20 to 2026-05-23)_
Brief calls for a 64px sidebar rail at tablet widths and a hamburger sheet at mobile widths.

**Done:**
- Hamburger drawer at ≤1024px (commit `7f2afeb`, 2026-05-20).
- Header condense, comparison stack, table scroll-shadow, release phone-block, staff/admin tablet advisory all landed in the same pass.
- **All 14 data tables** in the app now use `.table-wrap-cards` — at ≤767px every `<tr>` converts to a stacked card block with `data-label` column labels rendered via `td::before`. Covers: `dashboard.php`, `staff.claims.php`, `staff.dashboard.php` (×2), `found.php`, `found.show.php`, `matches.php`, `admin.audit.php`, `admin.donate.php`, `admin.its.php`, `admin.reports.php`, `admin.report.show.php` (×4), plus the earlier `lost.php` and `claims.php`. No plain `class="table-wrap"` divs remain in `pages/`. (2026-05-23)

**Still outstanding:**
- 64px icon sidebar rail at tablet widths — deferred pending icon-set decision. Text-only nav is the current fallback.

**Status:** browser-unverified. The layout is structurally correct per static analysis; human verification at 320 px, 375 px, and 768 px is needed to confirm WCAG 1.4.10 (Reflow) compliance.
