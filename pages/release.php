...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_verifiable) {
    ...
    // Verification checklist — staff attestation per DESIGN_BRIEF.md §9. The
    // gold "Confirm release" button is JS-disabled until all three are ticked,
    // but the server is the real gate (defence in depth).
    $required_checks = ['id_checked', 'identity_matches', 'item_matches'];
    $checked = array_values(array_intersect($required_checks, (array) ($_POST['verify'] ?? [])));
    if (count($checked) !== count($required_checks)) {
        $errors['verify'] = ['Confirm all three verification steps before releasing.'];
    }
        ...
        try {
            ...
            db_transaction(function () use (
                ...
                audit_log('claim.release', 'claim_ticket', $claim_id, [
                    ...
                    'verification' => ['id_checked', 'identity_matches', 'item_matches'],
                ]);
                ...
                // Notify claimant (subject to admin notification rules)
                notify(
                    (int) $claim['claimant_account_id'],
                    'claim.released',
    ...
    <?php if ($is_verifiable): ?>
      <section class="card" aria-labelledby="release-form-title">
        ...
        <form method="POST" enctype="multipart/form-data" class="stack-4" id="release-form">
          ...
          <fieldset class="release-checklist">
            <legend class="form-label form-label-required">Verification checklist</legend>
            <?= field_error_html('verify', $errors) ?>
            <label class="check-option">
              <input type="checkbox" name="verify[]" value="id_checked" data-release-check>
              <span>I checked the claimant's photo ID.</span>
            </label>
            <label class="check-option">
              <input type="checkbox" name="verify[]" value="identity_matches" data-release-check>
              <span>The claimant matches the reported owner on file.</span>
            </label>
            <label class="check-option">
              <input type="checkbox" name="verify[]" value="item_matches" data-release-check>
              <span>The item matches the claimant's description and photo.</span>
            </label>
          </fieldset>
          ...
          <button type="button" class="btn btn-accent" data-release-trigger
                  data-modal-open="release-confirm-modal">
            Confirm release
          </button>
          <span class="btn-hold-hint" data-release-hint aria-live="polite"></span>
          ...
          <!-- Hold-to-confirm modal (Task 27) — final guard before the release
               ...
               modal.js calls form.requestSubmit() (no submitter arg — the hold
               button is type="button", not type="submit") which runs native
               constraint validation then submits. -->
          <?php
          ...
          ?>
            ...
            <button type="button" class="btn btn-accent btn-hold" data-modal-hold="1500">
