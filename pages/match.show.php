...
page_header(
    'Match #' . $id,
    score_chip((int) $match['score']) . ' ' . status_badge((string) $match['status'])
);
?>
...
<?php if ($factors): ?>
  <section class="card" aria-labelledby="score-title">
    ...
    <table class="data-table score-breakdown-table">
      <thead>
...
<?php if ($is_actionable): ?>
  <section class="card" aria-labelledby="review-title">
    ...
    <form method="POST" class="stack-4">
      ...
      <div class="field">
        <label for="review_notes" class="field-label">Notes
          <span class="text-muted text-sm">(required when marking "Needs info")</span>
        </label>
        <textarea id="review_notes" name="review_notes" rows="4" class="field-textarea"
    ...
    <form method="POST" id="reject-match-form">
      ...
      ?>
        ...
        <div class="field">
          <label for="reject_reason" class="field-label field-label-required">
