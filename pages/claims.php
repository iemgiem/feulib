...
// Staff and admin manage the full claims queue — redirect them there.
if ($role === 'staff' || $role === 'admin') {
    go(url('/index.php?p=staff.claims'));
}
        ...
        <tbody>
          <?php foreach ($claims as $c): ?>
            <tr>
              ...
              <td class="col-narrow text-sm text-muted" data-label="Created">
                <?= e(time_ago((string) $c['claim_created_at'])) ?>
              </td>
              <td class="col-narrow" data-label="Status">
                <?= status_badge((string) $c['claim_status']) ?>
                <div class="text-sm text-muted" style="margin-top: var(--space-1);">
                  Item: <?= status_badge((string) $c['lost_status']) ?>
                </div>
              </td>
